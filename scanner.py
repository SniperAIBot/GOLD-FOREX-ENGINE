...
from config import SYMBOLS, COOLDOWN_MINUTES, LOSS_STREAK_LIMIT, PAUSE_AFTER_LOSS_STREAK_SECONDS
...
def should_pause_trading():
    ...
    if now < PAUSED_UNTIL:
        ...
        logger.warning(f"🛑 FOREX TRADING PAUSED AFTER LOSS STREAK. REMAINING={remaining_minutes} MIN")
        return True
    ...
    if len(losses) == LOSS_STREAK_LIMIT:
        ...
        logger.error(f"🛑 FOREX LOSS STREAK DETECTED: {LOSS_STREAK_LIMIT} LOSSES. PAUSING.")
        return True
    return False
...
def correlation_filter(signals, new_signal):
    if not signals:
        return True
    same_direction = sum(1 for signal in signals if signal["direction"] == new_signal["direction"])
    if same_direction >= 3:
...
def scan_market():
    ...
    for symbol in SYMBOLS:
        try:
            ...
            logger.info(f"🧠 FOREX AI PROFILE {symbol}: WR={profile['win_rate']}% TRADES={profile['trades']} WEIGHT={profile['weight']} MIN_CONF={profile['confidence_required']}")
            ...
            if candles_5m is None or candles_15m is None or candles_1h is None:
                logger.warning(f"⚠️ FOREX NO DATA: {symbol}")
                continue
            logger.info(f"📈 FOREX ANALYZING {symbol}")
            signal = analyze(symbol, candles_5m, candles_15m, candles_1h, profile, candles_4h)
            if signal is None:
            ...
            logger.info(f"🔥 FOREX SIGNAL FOUND {symbol} {signal['direction']} CONF={signal['confidence']} AI_WEIGHT={signal.get('ai_weight')} SMC_SCORE={signal.get('smc_score', 'N/A')} SMC_GRADE={signal.get('smc_grade', 'N/A')} SMC_CONFIRMED={signal.get('smc_confirmed', 'N/A')}")
